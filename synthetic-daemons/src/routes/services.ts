...
export function servicesRouter(store: MemoryStore, config: AppConfig): Router {
  ...
  router.get("/:service/running", (req: Request, res: Response) => {
    ndjsonResponse(res, store.listRunningServices(req.params.service));
  });
  ...
  router.post("/:service/cancreate", (_req: Request, res: Response) => {
    const servers = store.activeTargets
    ...
    json(res, {
      ...
      servers: servers.length > 0 ? servers : [
        { cancreate: true, provider: "scaleway", target: "scaleway-demo-public", carbon_footprint: null, billing: [] },
        { cancreate: true, provider: "ovh", target: "ovh-demo-public", carbon_footprint: null, billing: [] },
      ],
    });
  });
  ...
  router.post("/:service/create", (req: Request, res: Response) => {
    ...
    const record = store.createServiceInstance(
      serviceType,
      serviceUuid,
      billingUuid,
  ...
  router.delete("/:service/delete", (req: Request, res: Response) => {
    store.removeService(req.params.service, (req.body?.service_uuid as string) ?? "");
    ok(res);
  });
